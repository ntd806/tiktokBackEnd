...
export class BaseRepository<T> {
    ...
    async update<T>(id: string, entity: T) {
        return await this.baseModel.findByIdAndUpdate(id, { $set: entity },
            { new: true })
    }

    async updateMany<T>(filter: DynamicFilter, entity: T): Promise<any> {
        return await this.baseModel.updateMany(filter as any, { $set: entity },
            { multi: true })
    }
    ...
    async paginateRecordByField(filter: DynamicFilter, paging: Paging) {
        return await this.baseModel.find(filter as any)
            .skip(paging.offset)
            .limit(paging.limit)
    }
